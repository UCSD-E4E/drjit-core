...
static bool run_exec_arm(const Options &o, const std::string &kernel_src,
    ...
    // Specimen kernels self-check and write 0 on success, so one flag turns
    // "it ran" into "it computed the right thing" without golden-value files
    // (which would have to be generated by running the code they validate).
    if (o.expect_zero) {
        unsigned bad_idx = 0, bad_n = 0;
        for (unsigned i = 0; i < o.n; ++i)
            if (out[i] != 0.0f) { if (!bad_n) bad_idx = i; bad_n++; }
        if (bad_n) {
            printf("  [exec] FAIL  %u/%u lanes nonzero; first at [%u] = %g\n"
                   "               (specimen self-check reported an error code)\n",
                   bad_n, o.n, bad_idx, out[bad_idx]);
            return false;
        }
        printf("  [exec] PASS  %u elems, compile %.0f ms, all lanes self-checked 0\n",
               o.n, compile_ms);
        return true;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--expect-zero") o.expect_zero = true;
