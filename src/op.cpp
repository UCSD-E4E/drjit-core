...
uint32_t jitc_var_gather(uint32_t src_, uint32_t index, uint32_t mask) {
    ...
    if (!result && var_info.size == 1 && var_info.literal &&
        ...
        void *p = (uint8_t *) jitc_var(src)->data + size * pos,
             *p_out = jitc_malloc((JitBackend) src_info.backend, size);
...
uint32_t jitc_var_scatter(uint32_t target_, uint32_t value, uint32_t index_,
    ...
    if (needs_reduce) {
        // Counterpart to the expansion: narrow back to a regular array. Fires
        // for the LLVM 'Expand' reduce mode (the only backend setting
        // 'needs_reduce').
    ...
    if (needs_narrow)
        ...
        jitc_var_shadow_setup_narrow(scatter_op, target);
#endif
